...
/// This transaction removes access to a child account from the signer's LinkedAccounts Collection.
/// Note that the signer will no longer have access to the removed child account via AuthAccount Capability, so care
...
transaction(childAddress: Address) {
    ...
    let managerRef: &LinkedAccounts.Collection
    ...
    execute {
        ...
        self.managerRef.removeLinkedAccount(withAddress: childAddress)
    }
}
